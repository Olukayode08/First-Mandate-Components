...
const ActivateUser = () => {
  ...
  const {
    ...
    error,
    isSuccess,
  } = useFirstMandateMutation('/activate-from-addition', {
    onSuccess: (data) => {
      // updateToken(data)
      setTimeout(() => {
        navigate('/login')
  ...
  const handleNewPassword = async (e) => {
    ...
    try {
      await postNewPassword({ password, userTokenId })
    } catch (e) {
          ...
          <form onSubmit={handleNewPassword}>
            ...
            >
              {isLoading ? (
                ...
                <p className='login-btn'>Submit Password</p>
              )}
            </button>
          </form>
        </section>
      </ActivateU>
      <div>{isSuccess && <ActivateUserModal />}</div>
    </>
  )
}
const ActivateU = styled.section`
  section {
    position: relative;
  }
  .logo {
